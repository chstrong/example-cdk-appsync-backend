...
export class FileStorageStack extends Stack {
	constructor(scope: Construct, id: string, props: FileStorageStackProps) {
		...
		const fileStorageBucket = new s3.Bucket(this, 'S3Bucket', {
            bucketName: `${props.appName.toLowerCase()}-bucket-${props.stage.toLowerCase()}`,
		...
		const imagesCFDistribution = new Distribution(this, 'ImageDistribution', {
			defaultBehavior: {
